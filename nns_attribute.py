...
def tokenize_plus_nns(model, inputs: List[str], max_length: Optional[int] = None):
    """
    ...
    inputs = [model.tokenizer.bos_token + string for string in inputs]
    tokens = model.tokenizer(inputs, padding=True, padding_side='right', truncation=(max_length is not None), max_length = (model.config.n_ctx), return_tensors="pt")
    if max_length is not None:
    ...
    return tokens['input_ids'], attention_mask, input_lengths, n_pos


def make_hooks_and_matrices_nns(model, graph: Graph, batch_size:int , n_pos:int, scores: Optional[Tensor]):
    ...
    activation_difference = torch.zeros((batch_size, n_pos, graph.n_forward, model.config.hidden_size), device=model.device, dtype=model.dtype)
...
def compute_mean_activations_nns(model, graph: Graph, dataloader: DataLoader, per_position=False):
    ...
    for batch in tqdm(dataloader, desc='Computing mean'):
        ...
        tokens, attention_mask, input_lengths, n_pos = tokenize_plus(model, batch_inputs, max_length=512) #TODO
        total += len(batch_inputs)

        if not means_initialized:
            # here is where we store the means
            if per_position:
                means = torch.zeros((n_pos, graph.n_forward, model.cfg.d_model)) #TODO dtype and device
            else:
                means = torch.zeros((graph.n_forward, model.cfg.d_model))
            means_initialized = True

        with model.trace({"input_ids": tokens, "attention_mask": attention_mask}):
            pass
    ...
    with model.session(remote=True) as session:
        ...
        if per_position:
            means = torch.zeros((n_pos, graph.n_forward, model.cfg.d_model), device='cuda', dtype=model.cfg.dtype).save()
        else:
            means = torch.zeros((graph.n_forward, model.cfg.d_model), device='cuda', dtype=model.cfg.dtype).save()
        ...
        with session.iter(dataloader) as batch:
            ...
            with model.trace(tokens, attention_mask):
                for hook_point, index in hook_points_indices: # TODO
                    activations = model.hook_point.output.detach()
                    ...
                    if input_lengths is not None:
                        ...
                        item_means = einsum(activations, mask, 'batch pos ... hidden, batch pos ... hidden -> batch ... hidden')
